"""FastMCP HTTP+SSE server for the seller — exposes the cars-pack@1 tool surface.
...
class SessionBinding:
    """A buyer's MCP session bound by ``inquiry_listener``.

    The buyer's agent first sends a NIP-17 ``mcp_inquiry_open`` rumor
    carrying a ``session_token``. ``inquiry_listener`` decrypts that,
    binds ``session_token → buyer_pubkey`` here, and the FastMCP
    ...
    Attributes:
        session_token: Opaque token presented in the
            ``Authorization`` header on the buyer's MCP handshake.
        buyer_pubkey: Pubkey from the rumor signer; the policy keys
            user-confirm decisions off this.
        item_id: The listing the buyer asked about.
...
class SessionRegistry:
    ...
    For Week 1 this is a process-local dict. If the seller process
    restarts, in-flight sessions are lost — that's acceptable because
    the buyer will retry with a fresh inquiry rumor.
    """
    ...
    def take(self, session_token: str) -> SessionBinding | None:
        ...
        Args:
            session_token: The token from the buyer's handshake.
...
def build_server(
    *,
    ...
    Args:
        name: Server name (e.g. ``"chaos-seller"``).
        host: Bind host.
        port: Bind port.
...
#    the buyer's client surfaces back to the buyer.
...
def submit_offer_impl(
    ...
    *,
    ...
    """Record a buyer's offer and surface it to the user.

    Args:
        item_id: ``d`` tag of the listing.
        amount: Offer amount in the smallest currency unit (e.g. cents).
        currency: ISO 4217 code (``"EUR"`` etc.).
        note: Optional buyer-supplied note (sanitized before display).

    Returns:
        Acknowledgement text the buyer's agent will read.
    """
    raise NotImplementedError("mcp_server.submit_offer_impl not implemented")


def cancel_inquiry_impl(item_id: str, *, reason: str = "") -> str:
    """Mark the buyer's session as withdrawn.

    Args:
        item_id: ``d`` tag of the listing.
        reason: Optional buyer-supplied reason (sanitized).
...
async def serve(
    ...
    *,
    ...
    """Run the seller's FastMCP server until cancelled.
