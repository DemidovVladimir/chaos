...
|---|---|---|---|
| Photos returned by `request_photos` match stock library | fast | every `ImageContent` block in a `request_photos` tool result | hard suppress + warn user |
| Photos seen before in another listing by a different pubkey | fast | every `ImageContent` block in a `request_photos` tool result | hard suppress + warn user |
| Detail photos match stock at lower confidence | thorough | every `ImageContent` block in a `request_photos` tool result | warn user, lower score |
