...
Subcommands:
  ...
  python seller.py serve FILE            — publish a listing, start the FastMCP
                                           HTTP+SSE server in a background
                                           thread, and listen for inquiries in
                                           the foreground.
  python seller.py serve-multi DIR       — publish every *.toml in DIR as its
...
field in the listing TOML so buyers know where to reach it.

`serve-multi` is the multi-car variant: drop several listings into
`mvp/listings/` and run `python seller.py serve-multi listings/`.
...
def cmd_serve(toml_path: str) -> None:
    ...
    # Bring up MCP first so buyers calling immediately after seeing the
...
def main() -> None:
    ...
    if cmd == "keygen":
        ...
        if len(sys.argv) < 3:
            print("Usage: seller.py serve FILE.toml")
            sys.exit(2)
        cmd_serve(sys.argv[2])
    elif cmd == "serve-multi":
        if len(sys.argv) < 3:
            print("Usage: seller.py serve-multi DIR")
            sys.exit(2)
