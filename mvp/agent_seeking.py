"""
agent_seeking.py — weekend MVP seeking agent.

Subcommands:
  python agent_seeking.py keygen           — generate / show your seeking agent identity
  python agent_seeking.py watch            — subscribe to NIP-99 cars listings,
                                     prompt to DM the offering agent on each match,
                                     and after the offering agent's reply offer
                                     to fetch photos + inspection report
                                     via the offering agent's MCP HTTP+SSE server.

Edit MATCH_FILTER below to change what you're looking for.

After a NIP-99 match the seeking agent caches the listing's `["mcp", url]`
tag (if present) keyed by publisher pubkey. When the offering agent replies, we
...
# Per-offering-agent MCP context cached when we see a NIP-99 listing, so that
# when the offering agent replies we can prompt the user to MCP-fetch from the
# correct URL + item_id without round-tripping the relay again.
# Map: to_pubkey_hex → {"mcp_url": str, "item_id": str}
...
# refetch (e.g. offering agent said they uploaded new photos) the seeking agent can
...
def cmd_keygen() -> None:
    me = Identity.load_or_create("seeking")
    print(f"Seeking agent identity:")
...
def _handle_listing(ev: Event, sk: PrivateKey, rm: RelayManager) -> None:
    ...
    # Cache the offering agent's MCP URL + item_id keyed by pubkey, so that
    ...
    text = input("Send the offering agent a DM? Type a message (blank to skip):\n> ").strip()
    if not text:
        return
...
def _handle_reply(ev: Event, sk: PrivateKey) -> None:
    ...
    if fetch_key in _FETCHED:
        # Already pulled photos + inspection from this offering agent for this
        # item earlier in the session — don't re-prompt on every reply.
        # Run `python mcp_client.py <url> <item_id>` manually to refetch.
        return

    answer = input(
        f"This offering agent's listing advertised an MCP endpoint:\n"
