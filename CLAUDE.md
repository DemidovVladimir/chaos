...
You're working in **chaos**, a peer-to-peer marketplace built
on Nostr (discovery + identity) and MCP (agent-to-agent rich
...
more details, the buyer's agent opens an MCP session against the
seller's MCP server and receives photos and inspection PDFs as
`ImageContent` and `EmbeddedResource` blocks. The platform never
custodies funds, inventory, PII, or files. Cars vertical first;
other verticals (real estate, watches, etc.) follow once cars proves
out.
...
- **A second peer transport** — MCP is the canonical wire for
  buyer↔seller dialogue and binary content. Do not introduce ACP,
  A2A, gRPC, custom WebSocket protocols, or anything else as a
  parallel peer transport without explicit user override. Earlier
  iterations of the design considered ACP and A2A; both are
  superseded — see `spike/MCP_SPIKE_REPORT.md` for why.
...
- **In-app social features** beyond NIP-51 mute lists and NIP-58
  badges
- **Per-vertical paid tier — pro is cross-vertical via
...
- Top-level component folders: `seller/`, `buyer/` (universal
  ...
  `reputation/` (reputation/dispute architecture docs and kind
  registry); `operator/<vertical>/` (per-vertical operator infra-as-
  code — `operator/cars/` was previously `registry/`); `mvp/`;
...
- Per-vertical operator infra-as-code lives under
  `operator/<vertical>/` (the prior `registry/` directory was
  renamed to `operator/cars/` to make the per-vertical scope
  explicit; future verticals get `operator/realestate/`,
  `operator/watches/`, etc.)
...
A vertical pack (cars-pack, real-estate-pack, …) is what makes the
marketplace composable. Each pack defines:
