...
seller skill + buyer skill + custom MCP specs + an example listing.
It's the first vertical pack and the template every later vertical
pack will copy.
...
├── skills/
│   ├── seller-cars/SKILL.md    seller skill — listing, photos via ACP, inquiries
│   └── buyer-cars/SKILL.md     buyer skill — filter, evaluate, negotiate
└── mcp/
...
- **Seller and buyer skills** — Hermes skills tuned for car-specific
  flows: photo coverage checklist, ACP photo grant policy, financing
  conversation patterns, history-report etiquette.
- **Three custom MCPs** — local-only, no third-party data dependency:
...
- Photos and inspection PDFs move agent-to-agent over ACP — never
  through a third-party file host
- The reverse-image-mcp is sold as a paid add-on, runs on the
  buyer's own machine on bytes the buyer's agent received via ACP
