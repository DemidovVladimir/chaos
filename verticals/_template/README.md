...
A vertical pack is the contract every seller and buyer in this
...
1. `tag_schema.md` — which NIP-99 tags are required / recommended /
   ...
   the MCP tool surface contract every seller in this vertical must
   expose.
2. `example_listing.json` — a complete NIP-99 event a seller could
   publish today, with every required tag filled in.
3. `skills/<vertical>-seller/SKILL.md` — Hermes seller skill: hard
   rules, listing flow, MCP tool handlers, grant policy, negotiation
   flow, verification checklist.
4. `skills/<vertical>-buyer/SKILL.md` — Hermes buyer skill: filter
...
├── skills/
│   ├── <vertical>-seller/SKILL.md   fill in: hard rules + listing/MCP/negotiation flow
│   └── <vertical>-buyer/SKILL.md    fill in: filter + rubric + inquiry flow
└── mcp/
...
- `<DOMAIN_PHOTO_OR_DOC>` → the binary content kind your buyers ask
...
Also rename the two folders inside `skills/` from `<vertical>-seller`
and `<vertical>-buyer` to your concrete names.
...
- Photos and binary documents NEVER ride a public URL. They flow as
  ...
  tool call on the seller's own MCP server. If a payload exceeds
...
- No tag carries PII (full name, full address, phone, government
  IDs, payment instruments). Such fields stay on the seller's
...
- Local capability MCPs in `mcp/` may compute over data the user
  ...
  brokers' feeds.
- Sellers MAY expose more tools than the contract requires; buyers'
  skills surface unknown tools to the user as "this seller offers
  also: …".
