...
5. **Toolset**: `nostr_subscribe`, `nostr_dm_send`, `mcp_connect`
   (open an MCP HTTP+SSE session to a seller's `mcp` tag URL),
   `mcp_call_tool` (invoke any tool from `tools/list` on a connected
   seller), `reverse_image_check`, `vin_decode`, `market_comp`,
   ...
   (limited to `~/.chaos/buyer/`). Note: `mcp_serve` is
   forbidden in buyer plugins per Rule 11.
...
```
reputation_mcp.get_reputation(seller_pubkey, "cars-pack@1")
```
...
## Inquiry flow — connecting to the seller's MCP server

When the user says "ask the seller about this":

1. **Open Nostr handshake**: send a NIP-17 sealed gift-wrap to the
   seller with a small `mcp_inquiry_open` payload (item_id, buyer
   pubkey, session_token). This tells the seller's agent "expect an
   MCP connection from this buyer."

2. **Connect to seller's MCP server**: use `mcp_connect` with the
   URL from the listing's `["mcp", ...]` tag.

3. **Bootstrap — `tools/list`**: the seller may expose more than
...
4. **Default asks via MCP tool calls**:
   ...
   - `request_vin(item_id)` → text or denial (likely user-confirm on seller side)
...
2. Show the user the suggested offer with the rationale ("median is
   ...
   adjust.
3. Call seller's `submit_offer(item_id, price_cents, conditions)`
...
When a deal goes wrong in a way the buyer wants on record — and
where the seller's behavior may matter to other buyers — call
`report_to_admin(conversation_id, complaint_text)`. Typical
scenarios:

- **Bait-and-switch** — seller's MCP-delivered photos and
  description don't match the public listing or the car the buyer
  inspected on-site.
- **Undisclosed major damage** — inspection at the buyer's chosen
  shop reveals collision repair the listing claimed
  `accident_history: none_known`.
- **Seller refused inspection then disappeared** — seller declined
  `request_inspection_at_shop`, conversation went cold, listing
  was relisted under a different pubkey.
- **Payment dispute** — disagreement after handover (buyer paid,
  car never delivered; or seller delivered a different car than
  the one negotiated).

Mechanics: same as the seller side. `report_to_admin` packages the
conversation log + any associated attestations + the buyer's
complaint text, NIP-44-encrypts the bundle to the admin-pubkey,
signs it with the buyer's key, mines NIP-13 PoW (24 bits), and
...
the buyer; admin's verdict is a signal, not a court ruling — the
seller has appeal via kind 30431.
...
- Seller has admin-decision flag from a trusted admin in the last
  90 days
