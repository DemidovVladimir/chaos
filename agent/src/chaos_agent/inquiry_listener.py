...
seeking agent pubkey for the upcoming MCP session.

The rumor type is ``mcp_inquiry_open``; it carries:

- ``session_token`` — opaque opaque token the seeking agent's agent will
  present in the ``Authorization`` header on the MCP HTTP+SSE handshake.
- ``item_id`` — the listing the seeking agent is interested in.
- ``from_pubkey`` — derived from the rumor signer; the MCP server
  binds the session_token → from_pubkey before accepting any tool
  call on that session.
- ``free_text`` (optional) — anything the seeking agent wrote; passed through
...
The MVP path (`mvp/agent_offering.py`) uses NIP-04 plaintext DMs as an
...
class Inquiry:
    """Decrypted, sanitized ``mcp_inquiry_open`` rumor from a seeking agent.
    ...
    Attributes:
        ...
        session_token: Opaque token the seeking agent's agent will present on
                       the MCP HTTP+SSE handshake. The offering agent's MCP
                       server binds this token → ``sender_pubkey``
                       before accepting any tool call.
        free_text: Optional free-form text from the seeking agent; sanitized.
...
def listen(
    ...
    *,
    ...
    Args:
        ...
        my_sk_hex: Offering agent's secret key hex.
        my_pk_hex: Offering agent's pubkey hex.
    Yields:
...
def unwrap_gift(ev: Event, *, my_sk_hex: str) -> tuple[str, str]:
    ...
    Args:
        ...
        my_sk_hex: Offering agent's secret key hex (used for NIP-44 ECDH).
...
def send_reply(
    ...
    *,
    ...
    Args:
        rm: Live relay manager.
        my_sk_hex: Offering agent's secret key hex (signs the rumor).
        recipient_pubkey_hex: Seeking agent's pubkey from the original
