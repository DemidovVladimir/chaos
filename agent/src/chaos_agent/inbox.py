"""Append-only conversation log under ``~/.chaos/seeking agent/inbox/``.

One JSONL file per conversation:
``~/.chaos/seeking agent/inbox/<conversation_id>.jsonl``.

Each line is a JSON object: ``{ts, role, kind, payload, ...}``.
Per-conversation entries record both NIP-17 rumor exchanges (the
inquiry + reply) and per-MCP-tool-call audit rows: when the seeking agent
opens an MCP HTTP+SSE session against the offering agent's ``["mcp", url]``
...
``~/.chaos/seeking agent/inbox/<conversation_id>/{photos,documents}/``
...
DEFAULT_INBOX_DIR = Path.home() / ".chaos" / "seeking agent" / "inbox"
...
class InboxEntry:
    ...
    Attributes:
        ...
        ts: Unix seconds.
        role: ``"me"`` (seeking agent) or ``"offering agent"``.
