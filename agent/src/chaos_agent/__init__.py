"""Hermes plugin entry point for the chaos offering agent.
...
schemas to their handlers, ships the offering-cars skill, and registers
the ``hermes chaos-agent`` CLI subcommand.
...
AGENTS.md rules enforced here:
    ...
    - Rule 3: identity stays in ``~/.chaos/keys/agent.key``.
...
def register(ctx) -> None:
    ...
    offering agent's tool surface (cars-pack@1) is bootable from the same
    ...
    raise NotImplementedError(
        "chaos-agent register(ctx) not implemented yet — see offering agent/IMPLEMENTATION_PLAN.md."
    )
