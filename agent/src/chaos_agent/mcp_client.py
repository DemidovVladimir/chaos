"""MCP HTTP+SSE client — receives photos and PDFs from a offering agent's MCP server.

The seeking agent is purely an MCP CLIENT. Given a matched listing's
``["mcp", url]`` tag, this module dials the offering agent's MCP server over
HTTP+SSE, runs the standard ``initialize`` handshake, calls
``tools/list`` to bootstrap the offering agent's tool surface, then dispatches
...
async def connect(url: str) -> ClientSession:
    """Open an MCP HTTP+SSE session to the offering agent's MCP URL.
...
async def list_tools(session: ClientSession) -> list[Any]:
    ...
    Per the MCP spike, the seeking agent learns the offering agent's full tool
    ...
    Returns:
        The list of ``mcp.types.Tool`` objects the offering agent advertises.
    """
