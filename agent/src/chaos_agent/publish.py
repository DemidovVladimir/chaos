...
def build_event(
    ...
    *,
    ...
    - ``["mcp", <https-url>]`` — public URL of the offering agent's FastMCP
      HTTP+SSE server. A seeking agent's agent will dial this endpoint after
      sending a NIP-17 ``mcp_inquiry_open`` rumor.
    - ``["pack", <name>@<version>]`` — vertical pack contract the
      offering agent implements (e.g. ``cars-pack@1``). The seeking agent's skill
      uses this to know which tools to call.

    Args:
        item: The local item to publish.
        pubkey_hex: Offering agent's x-only pubkey hex.
        mcp_url: Public URL of the offering agent's MCP server. Must be
...
def publish(rm: RelayManager, sk_hex: str, raw_event: dict[str, Any]) -> Event:
    ...
    Args:
        ...
        sk_hex: Offering agent's secret key hex.
...
def deletion_request(
    ...
    Args:
        ...
        pubkey_hex: Offering agent's pubkey.
