...
An attestation is a small Nostr event the offering agent signs to assert a
...
Seeking agents' agents verify these signatures against the offering agent's pubkey
to weight evaluation. The seeking-cars SKILL.md uses this as a green
flag input.

We use ``kind: 30078`` (NIP-78 application-specific data) with a
namespacing ``["d", "chaos:attestation:<topic>"]`` tag so
relays can address them and the seeking agent can find them by topic.
...
class Attestation:
    ...
    Attributes:
        ...
        to_pubkey: Offering agent's pubkey hex.
...
def build(item_id: str, topic: str, body: str, *, to_pubkey_hex: str) -> dict:
    ...
    Args:
        ...
        to_pubkey_hex: Offering agent's pubkey hex.
