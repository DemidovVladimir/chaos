"""CLI entry: ``hermes chaos-agent {publish, listen, serve, status}``.

The offering agent plugin registers a CLI subcommand via ``ctx.register_cli_command``
...
- ``keygen`` — generate the offering agent keypair if it doesn't exist.
"""
...
def setup_argparse(subparser: argparse.ArgumentParser) -> None:
    """Build the argparse tree for ``hermes chaos-agent``.
    ...
    p_keygen = subs.add_parser("keygen", help="Show / generate the offering agent keypair")
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    if func is None or func is dispatch:
        print("Usage: hermes chaos-agent {keygen,publish,listen,serve,status}")
        return
    func(args)


def cmd_keygen(args: argparse.Namespace) -> None:
    """Print or create the offering agent identity.
