"""Publisher-side configuration loaded from ``~/.chaos/offering agent.yaml``.

The offering agent config tells the plugin which relays to talk to, where its
...
DEFAULT_CONFIG_PATH = Path.home() / ".chaos" / "offering agent.yaml"
...
class GrantPolicyConfig:
    ...
    defaults_from: str = "verticals/cars-pack/skills/offering-cars/SKILL.md"
...
@dataclass(frozen=True, slots=True)
class SellerConfig:
    """Top-level offering agent config.
