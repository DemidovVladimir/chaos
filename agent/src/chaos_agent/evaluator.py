"""Apply the seeking-cars rubric to an incoming listing.

The full rubric lives in
``verticals/cars-pack/skills/seeking-cars/SKILL.md`` § "Evaluation rubric".
...
returned from ``mcp_call_tool("request_photos", ...)`` on the offering agent's
...
def evaluate_seller_reputation(pubkey_hex: str, *, seen_cache_path: object) -> list[Flag]:
    ...
    previously-seen listings (offering agent's pubkey age, prior listing
