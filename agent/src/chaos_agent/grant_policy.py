...
The offering agent's MCP server (see ``mcp_server.py``) consults this module
on every ``tools/call`` from a seeking agent's agent. Each cars-pack@1 tool
...
- ``ASK_USER`` — block until the user explicitly grants this tool
  call for this specific seeking agent.
