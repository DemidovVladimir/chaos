...
An *item* is everything the offering agent has on disk for one car:
...
- ``private/vin.txt`` — full VIN, never leaves the user's machine
  unless explicitly granted per-seeking agent (via ``request_vin``).
...
class Item:
    ...
    Attributes:
        ...
        accepts_offer: Whether the offering agent is open to counter-offers.
...
class Catalog:
    ...
    the offering agent plugin ever grows multi-process write paths, switch to
    flock'd JSON.
    """
