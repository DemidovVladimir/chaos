...
These are the publisher-side Hermes tools the offering-cars skill calls
...
- ``handle_inquiry`` walks every pending MCP tool call from a seeking agent's
...
def handle_inquiry(args: dict, **kwargs: Any) -> str:
    ...
    1. Look up the open MCP session by ``session_token`` (bound to a
       seeking agent pubkey by ``inquiry_listener``).
