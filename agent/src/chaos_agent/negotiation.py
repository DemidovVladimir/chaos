"""Negotiation state machine for the offering agent side.
...
Per ``verticals/cars-pack/skills/offering-cars/SKILL.md`` § "Negotiation flow",
the offering agent compares the seeking agent's offer against the user's
...
class Offer:
    ...
    Attributes:
        actor: ``"seeking agent"`` or ``"offering agent"``.
...
class Match:
    ...
    Attributes:
        item_id: The listing.
        counterparty_pubkey: The seeking agent's pubkey hex.
...
def evaluate_buyer_offer(
    ...
    *,
    ...
    """Suggest an action given a new seeking agent offer.

    Per the offering-cars SKILL.md flow:
    ...
    Args:
        match: The current match state.
        new_offer: A seeking agent-actor offer to evaluate.
