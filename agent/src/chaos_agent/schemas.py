...
PUBLISH_ITEM = {
    ...
    "description": (
        ...
        "the user's local items to the offering agent's configured Nostr "
        "relays. Builds the event from the local manifest, mines "
        "≥ 20-bit NIP-13 PoW, signs with the offering agent's identity, and "
...
HANDLE_INQUIRY = {
    "name": "handle_inquiry",
    "description": (
        "Process a NIP-17 inquiry from a seeking agent. Decrypts the "
        "mcp_inquiry_open rumor, sanitizes free text, binds the "
        "seeking agent's session_token to their pubkey, and runs each "
...
GRANT_ASKS = {
    ...
    "description": (
        ...
        "calls for a specific seeking agent's session. Use only when the "
        "user has confirmed they want this seeking agent's agent to receive "
...
COUNTER_OFFER = {
    ...
    "parameters": {
        ...
        "required": ["item_id", "from_pubkey", "amount_cents", "currency"],
    },
}
...
ACCEPT_OFFER = {
    "name": "accept_offer",
    "description": (
        "Accept the seeking agent's most recent offer. ALWAYS requires "
        ...
        "update_item call after the seeking agent's confirmation."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
REJECT_OFFER = {
    "name": "reject_offer",
    "description": (
        "Reject the seeking agent's most recent offer with a brief reason. "
    ...
    "parameters": {
        ...
        "required": ["item_id", "from_pubkey", "reason"],
    },
}
