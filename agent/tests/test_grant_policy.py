"""Tests for ``chaos_agent.grant_policy``.

The default decision table MUST stay aligned with
``verticals/cars-pack/skills/offering-cars/SKILL.md`` § "Inquiry-handling
...
def test_default_grants_match_skill_md() -> None:
    """Every cars-pack@1 tool in offering-cars SKILL.md is reflected in DEFAULT_TOOL_TABLE."""
    pytest.skip("not yet implemented")
def test_request_vin_always_asks_user() -> None:
    """``decide("request_vin", {}, item)`` returns ASK_USER, never GRANT.

    Hard rule from offering-cars SKILL.md: never auto-share full VIN.
    Only after explicit user approval per seeking agent.
    """
    pytest.skip("not yet implemented")
