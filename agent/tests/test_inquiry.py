...
def test_inquiry_payload_shape_matches_protocol() -> None:
    ...
    ``item_id``, ``from_pubkey``, ``session_token``, and ``asks``;
    no offering agent MCP URL is embedded (that comes from the listing's
    ``["mcp", url]`` tag).
    """
...
def test_inquiry_does_not_carry_seller_mcp_url() -> None:
    """The rumor payload must NOT include the offering agent's MCP HTTP+SSE URL.

    The seeking agent learns it from the matched NIP-99 listing's
