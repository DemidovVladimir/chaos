"""Tests for ``chaos_agent.mcp_server``.

We test the publisher-side FastMCP HTTP+SSE server with an in-process
...
def test_mcp_server_view_listing_returns_summary() -> None:
    ...
    The first thing a seeking agent's agent calls after ``tools/list``; the
...
def test_mcp_server_rejects_unknown_session_token() -> None:
    ...
    The seeking agent's agent must first send a NIP-17 ``mcp_inquiry_open``
...
def test_mcp_server_no_http_egress() -> None:
    """The offering agent's MCP path never makes an outbound HTTP call.
...
def test_mcp_server_two_machine_round_trip() -> None:
    """Offering agent and seeking agent plugins on different processes exchange a 3-photo session."""
    pytest.skip("not yet implemented")