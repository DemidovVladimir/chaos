"""Tests for ``chaos_agent.mcp_client``.
...
def test_list_tools_advertises_cars_pack_surface() -> None:
    """``list_tools`` against a cars-pack@1 offering agent returns the documented tool names."""
    pytest.skip("not yet implemented")
@pytest.mark.skip(reason="Requires a live MCP HTTP+SSE offering agent — integration test")
def test_mcp_client_two_machine_round_trip() -> None:
    """Seeking agent client + offering agent MCP server on different processes complete a full ask."""
    pytest.skip("not yet implemented")